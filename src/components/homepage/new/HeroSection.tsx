import React from 'react';
import { Play, ArrowRight } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { useNavigate } from 'react-router-dom';

const HeroSection = () => {
  const navigator = useNavigate();

  const handleMoveWorkSpace = () => {
    navigator("/vizualizer");
  };

  return (
    <section className="py-20 relative overflow-hidden lg:h-[100vh] md:h-[100vh] flex items-center">
      <div className="absolute inset-0 opacity-20 bg-[url('/assets/marble/hero-images.png')] bg-cover bg-center pointer-events-none" />

      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center">
          {/* Heading */}
          <h1 className="text-5xl md:text-6xl font-bold text-marble-charcoal mb-6 leading-tight">
            Plan your home renovation with{' '}
            <span className="bg-gradient-to-r from-blue-700 to-blue-400 bg-clip-text text-transparent">
              AI
            </span>
          </h1>

          {/* Subheading */}
          <p className="text-xl text-marble-gray-dark mb-8 max-w-3xl mx-auto leading-relaxed">
            Transform your space in seconds with AI-powered design. Upload a photo, describe your vision,
            and watch our advanced AI create stunning renovations that bring your dreams to life.
          </p>

          {/* CTA Buttons */}
          <div className="flex flex-col sm:flex-row gap-4 justify-center mb-16">
            <Button
              size="lg"
              className="bg-gradient-to-r from-blue-700 to-blue-400 hover:from-blue-800 hover:to-blue-500 text-lg px-8 py-4"
              onClick={handleMoveWorkSpace}
            >
              Start Designing Now
              <ArrowRight className="ml-2 h-5 w-5" />
            </Button>

            <Button
              size="lg"
              variant="outline"
              className="text-lg px-8 py-4 border-blue-700 text-blue-700 hover:bg-blue-700 hover:text-white"
            >
              <Play className="mr-2 h-5 w-5" />
              Watch Demo
            </Button>
          </div>

          {/* Stats Section */}
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8 max-w-4xl mx-auto">
            <div className="text-center">
              <div className="text-3xl font-bold text-blue-700 mb-2">1K+</div>
              <div className="text-marble-gray-dark">Renovations Daily</div>
            </div>
            <div className="text-center">
              <div className="text-3xl font-bold text-blue-700 mb-2">50+</div>
              <div className="text-marble-gray-dark">Design Styles</div>
            </div>
            <div className="text-center">
              <div className="text-3xl font-bold text-blue-700 mb-2">10M</div>
              <div className="text-marble-gray-dark">Satisfied Customers</div>
            </div>
            <div className="text-center">
              <div className="text-3xl font-bold text-blue-700 mb-2">4.8</div>
              <div className="text-marble-gray-dark">App Store Rating</div>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
};

export default HeroSection;
